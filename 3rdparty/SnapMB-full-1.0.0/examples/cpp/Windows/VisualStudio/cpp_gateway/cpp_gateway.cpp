...
int SNAP_API PasstroughHandler(void* usrPtr, byte DeviceID, void* RxPDU, word RxPDUSize, void* TxPDU, word& TxPDUSize)
{
    ...
    int Result = InnerClient->RawRequest(DeviceID, RxPDU, RxPDUSize, TxPDU, TxPDUSize, 0);

    // Converts Timeout and Hardware errors to be full Modbus compliant
    if (Result != mbNoError)
    {
        // Timeout
        if ((Result & 0x0FFFFFFF) == 0x00050007)
        {
            *pbyte(TxPDU) = *pbyte(RxPDU) + 0x80; // Modbus error
            *(pbyte(TxPDU) + 1) = errGatewayTargetFailed; // Target failed to respond
            TxPDUSize = 2;
        }
        ...
        {
            *pbyte(TxPDU) = *pbyte(RxPDU) + 0x80; // Modbus error
            *(pbyte(TxPDU) + 1) = errSlaveDeviceFailure;
            TxPDUSize = 2;
        }
    }

    if (TxPDUSize > 0)
        return int(TIndicationResult::irSendAnswer); // same of returning 2   
    else
        return int(TIndicationResult::irNoAnswer);   // same of returning 3
}
...
int main(int argc, char* argv[])
{
    ...
    InnerClient = new TSnapMBBroker(FormatRTU, "COM3", 19200, 'E', 8, 1, 0);
